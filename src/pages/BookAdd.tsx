...
export default function BookAdd() {
  ...
  async function save() {
    ...
    await supabase.from('gl_books').insert({
      title: title.trim(),
      author: author || null,
      year,
      ...
      review: review || null,
      added_at: now,
      started_at: status === '在读' ? now : null,
      finished_at: status === '读过' ? now : null,
    });
    setSaving(false);
    navigate('/books');
  }
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-bold text-gray-100 flex-1">添加书籍</h1>
      </div>
